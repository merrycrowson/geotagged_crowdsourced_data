# Code to calculate AUC for the best Wikipedia model
...
#### Calculate AUC for the model of the Wikipedia data ####------------------------------------------------------------------------------
...
my_test_pred_10 = get_logistic_pred(wiki.bin_best_model, data = mydata_tst, res = "wiki_T_F", 
                                    pos = 1, neg = 0, cut = 0.1)
my_test_pred_50 = get_logistic_pred(wiki.bin_best_model, data = mydata_tst, res = "wiki_T_F", 
                                    pos = 1, neg = 0, cut = 0.5)
my_test_pred_90 = get_logistic_pred(wiki.bin_best_model, data = mydata_tst, res = "wiki_T_F", 
                                    pos = 1, neg = 0, cut = 0.9)

# Evaluate accuracy, sensitivity, and specificity
my_test_tab_10 = table(predicted = my_test_pred_10, actual = mydata_tst$wiki_T_F)
my_test_tab_50 = table(predicted = my_test_pred_50, actual = mydata_tst$wiki_T_F)
my_test_tab_90 = table(predicted = my_test_pred_90, actual = mydata_tst$wiki_T_F)
...
my_test_prob = predict(wiki.bin_best_model, newdata = mydata_tst, type = "response")
head(my_test_prob)

my_test_roc = roc(mydata_tst$wiki_T_F ~ my_test_prob, plot = TRUE, print.auc = TRUE)

Wikipedia_model_AUC <- as.numeric(my_test_roc$auc) 

Wikipedia_model_AUC #A good model will have a high AUC, that is as often as possible a high sensitivity and specificity.
